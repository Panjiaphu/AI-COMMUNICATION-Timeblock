(function messagingCapabilitySurfacesV2(global) {
  ...
  const pttButton = panel?.querySelector("[data-ptt-record]");
  const pttLabel = panel?.querySelector("[data-ptt-record-label]");
  const pttFeedback = panel?.querySelector("[data-ptt-feedback]");
  ...
  const copyTable = {
    vi: {
      ...
      pttStart: "Bắt đầu nói",
      pttStop: "Dừng và gửi",
      pttProcessing: "Đang xử lý PTT",
    },
    en: {
      translate: "Translate",
      ...
      pttStart: "Start talking",
      pttStop: "Stop and send",
      pttProcessing: "Processing PTT",
    },
    "zh-TW": {
      translate: "翻譯",
      translating: "翻譯中…",
  ...
  function syncPttPresentation() {
    if (!pttButton || !pttLabel) return;
    const pressed = pttButton.getAttribute("aria-pressed") === "true";
    const disabled = pttButton.disabled;
    const state = disabled ? "processing" : pressed ? "recording" : "idle";
    const label = disabled ? text("pttProcessing") : pressed ? text("pttStop") : text("pttStart");
    pttButton.dataset.enterprisePttState = state;
    pttButton.dataset.enterprisePttInteraction = "toggle";
    pttButton.setAttribute("aria-label", label);
    setTextIfChanged(pttLabel, label);
    if (pttFeedback) {
      if (!pttFeedback.id) pttFeedback.id = "messaging-enterprise-ptt-feedback";
      pttButton.setAttribute("aria-describedby", pttFeedback.id);
    }
  }

  function enhancePttPresentation() {
    if (!pttButton || !pttLabel) return;
    app.dataset.copyPttHold = text("pttStart");
    pttObserver?.disconnect();
    pttObserver = new MutationObserver(syncPttPresentation);
    pttObserver.observe(pttButton, {
      subtree: true,
      childList: true,
      characterData: true,
      attributes: true,
  ...
  function getMetrics() {
    ...
    return {
      ...
      ptt_runtime_present: Boolean(pttButton),
      ptt_state: pttButton?.dataset.enterprisePttState || "unavailable",
      ptt_interaction: pttButton?.dataset.enterprisePttInteraction || "",
    };
  }
