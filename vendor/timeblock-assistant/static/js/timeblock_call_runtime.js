(() => {
  ...
  function broadcast(type) {
    try { state.channel?.postMessage({ type, tabId, at: Date.now() }); }
    catch (_error) { /* noop */ }
  }
  ...
  async function ringbackPulse() {
    if (!state.leader || !state.outgoing || state.incoming) return;
  ...
  function startRingback(call = state.outgoing) {
    ...
    ringbackPulse();
    state.ringbackTimer = setInterval(ringbackPulse, 3200);
  }
  ...
  function stopRingback() {
    ...
    if (typeof queueMicrotask === "function") queueMicrotask(disconnectNodes);
    else Promise.resolve().then(disconnectNodes).catch(() => undefined);
  }
