...
(() => {
  ...
  const qa = {
    ...
    reconnectTokenSeen: false, reconnectTokenRotationCount: 0, fastTimers: false,
    failFutureSockets: false,
  };
  ...
  const summarize = (raw, direction) => {
    try {
      ...
      const summary = {
        direction, event_name: message.event_name || 'unknown', event_version: message.event_version ?? null,
        session_id: message.session_id || null, participant_id: message.participant_id || null,
        sequence_number: message.sequence_number ?? null, connection_id: message.connection_id || null,
      ...
      if (message.event_name === 'session.authorized' && typeof message.reconnect_token === 'string') {
        ...
        if (message.reconnect_token.length > 0) {
          ...
          if (qa.lastReconnectToken !== null && qa.lastReconnectToken !== message.reconnect_token) qa.reconnectTokenRotationCount += 1;
  ...
  window.WebSocket = new Proxy(NativeWebSocket, {
    construct(Target, args) {
      ...
      socket.addEventListener('open', () => { if (qa.failFutureSockets) socket.close(4000, 'qa_forced_reconnect_failure'); });
      return socket;
    },
  });
  ...
  window.RTCPeerConnection = new Proxy(NativePeerConnection, {
    construct(Target, args) {
      const peer = new Target(...args); qa.peers.push(peer);
      ...
      peer.addEventListener('track', (event) => { const tracks = event.streams[0]?.getTracks() || [event.track]; for (const track of tracks) qa.remoteTrackEvents.push(track.id); });
      record(); return peer;
    },
  });
  if (navigator.mediaDevices?.getUserMedia) {
    ...
    navigator.mediaDevices.getUserMedia = async (...args) => {
      ...
      const stream = await nativeGetUserMedia(...args); qa.localTracks.push(...stream.getTracks()); return stream;
    };
  }
  ...
  window.__guiluaQa = {
    ...
    setSyntheticCaptions(sourceText, translatedText) { document.getElementById('source-caption').textContent = sourceText; document.getElementById('translated-caption').textContent = translatedText; },
    snapshot() {
      const localVideo = document.getElementById('local-video'); const remoteVideo = document.getElementById('remote-video');
      ...
      const localVideoTracks = localVideo?.srcObject?.getTracks() || []; const remoteVideoTracks = remoteVideo?.srcObject?.getTracks() || [];
      const remoteVideoTrackIds = remoteVideoTracks.map((track) => track.id);
      return {
        websocket_count: qa.websockets.length, websocket_states: qa.websockets.map((socket) => socket.readyState),
        websocket_urls: qa.websockets.map((socket) => socket.url), socket_closes: qa.socketCloses,
        inbound: qa.inbound, outbound: qa.outbound, peer_count: qa.peers.length, peer_states: peerStates,
        active_non_closed_peer_count: peerStates.filter((peer) => peer.connection_state !== 'closed').length,
        peer_history: qa.peerHistory, remote_track_ids: [...new Set(qa.remoteTrackEvents)], remote_track_event_count: qa.remoteTrackEvents.length,
        local_track_states: qa.localTracks.map(summarizeTrack), local_video_tracks: localVideoTracks.map(summarizeTrack),
        remote_video_tracks: remoteVideoTracks.map(summarizeTrack), duplicate_remote_video_track_ids: new Set(remoteVideoTrackIds).size !== remoteVideoTrackIds.length,
        remote_video_width: remoteVideo?.videoWidth || 0, remote_video_height: remoteVideo?.videoHeight || 0,
        ...
        reconnect_token_seen: qa.reconnectTokenSeen, reconnect_token_rotation_count: qa.reconnectTokenRotationCount,
        active_timeout_count: qa.activeTimeouts.size, reconnect_timer_active: qa.activeTimeouts.size > 0,
