...
def test_responsive_surface_and_interpreter_states(
    ...
    try:
        page.goto(_url(base_url, f"browser-ui-{name}", f"ui-{name}"), wait_until="networkidle")
        assert "token=" not in page.url
        expect(page).to_have_title("Timeblock AI Communication")
        expect(page.locator("#start-call")).to_be_enabled()
        for selector in (
            "#connection-pill", "#remote-video", "#local-video", "#translated-caption",
            "#interpreter-panel", "#source-language", "#target-language", "#start-call",
            "#microphone-toggle", "#camera-toggle", "#end-call", "#call-error",
        ):
        ...
        collapse.click(); expect(collapse).to_have_attribute("aria-expanded", "false")
        expect(page.locator("#interpreter-content")).not_to_be_visible()
        collapse.click(); expect(collapse).to_have_attribute("aria-expanded", "true")
...
def test_webkit_mobile_surface(webkit_browser, base_url: str, artifact_dir: Path):
    ...
    page = context.new_page(); evidence = observe_page(page)
    try:
        page.goto(_url(base_url, "webkit-mobile", "webkit-a"), wait_until="networkidle")
        assert "token=" not in page.url
        ...
        screenshot.parent.mkdir(parents=True, exist_ok=True); page.screenshot(path=str(screenshot))
        assert relevant_console_errors(evidence) == []
    finally:
        context.close()
...
def test_fake_media_permission_granted_and_cleanup(chromium_browser, base_url: str, artifact_dir: Path):
    context = create_context(chromium_browser, base_url=base_url, viewport={"width": 1366, "height": 768}, grant_media=True)
    page = context.new_page(); install_instrumentation(page); evidence = observe_page(page)
    try:
        page.goto(_url(base_url, "media-granted", "media-a"), wait_until="networkidle")
        assert "token=" not in page.url
        ...
        assert "token=" not in socket_url and "reconnect_token=" not in socket_url
        page.locator("#microphone-toggle").click(); page.locator("#camera-toggle").click()
        ...
        page.locator("#end-call").click(); expect(page.locator("#connection-label")).to_have_text("Ended")
        page.wait_for_function("window.__guiluaQa.snapshot().local_track_states.length === 2 && window.__guiluaQa.snapshot().local_track_states.every(track => track.ready_state === 'ended')")
        ...
        screenshot.parent.mkdir(parents=True, exist_ok=True); page.screenshot(path=str(screenshot))
        write_json(artifact_dir / "evidence" / "fake-media-granted.json", {"snapshot": snapshot, "page_url_token_free": True, "websocket_url_token_free": True, "console": evidence})
        assert relevant_console_errors(evidence) == []
    finally:
        context.close()
...
def test_synthetic_permission_denied_is_recoverable(chromium_browser, base_url: str, artifact_dir: Path):
    ...
    page = context.new_page(); install_instrumentation(page, deny_media=True); evidence = observe_page(page)
    try:
        page.goto(_url(base_url, "media-denied", "media-denied"), wait_until="networkidle")
        assert "token=" not in page.url
        page.locator("#start-call").click()
        ...
        write_json(artifact_dir / "evidence" / "permission-denied.json", {"snapshot": snapshot, "page_url_token_free": True, "console": evidence})
        assert relevant_console_errors(evidence) == []
    finally:
        context.close()
