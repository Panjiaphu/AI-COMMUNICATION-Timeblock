...
class FastApiSmokeTest(unittest.TestCase):
    ...
    def test_home_renders_bilingual_shell(self):
        ...
        self.assertNotIn("/bo?lang=zh-TW", response.text)
    ...
    def test_home_renders_rate_reference_mode(self):
        ...
        self.assertNotIn("/bo?lang=vi", response.text)
        self.assertNotIn("BO Trading", response.text)
        self.assertNotIn("Member & đại lý", response.text)
        self.assertNotIn("Hệ thống vận hành", response.text)
        self.assertIn("/member/transactions/send-home", response.text)
        self.assertIn("/member/transactions/buy-usdt", response.text)
        self.assertIn("/member/transactions/sell-usdt", response.text)
    ...
    def test_slbo_public_rooms_render(self):
        bo = self.client.get("/bo?lang=en", follow_redirects=False)
        self.assertEqual(bo.status_code, 303)
        self.assertEqual(bo.headers["location"], "/?lang=en")
