(function () {
  ...
  function buildConversationSheet() {
    ...
    pinned.type = "button";
    pinned.className = "messaging-mobile-sheet-action";
    pinned.textContent = actionLabel("v2PinnedOpen", "Pinned messages");
    pinned.addEventListener("click", () => {
      const source = messagesPanel.querySelector("[data-pinned-center-trigger]");
      safeDialogClose(conversationSheet);
      if (!source || source.disabled || source.closest("[hidden]")) return;
      source.click();
    });
    ...
    conversationSheet.append(heading, settings, pinned, translation, security, securityCopy, close);
  ...
  function bindDelegatedMessageInteractions() {
    ...
    threadMessages.addEventListener("pointerdown", (event) => {
      if (!isImmersiveConversation() || (event.pointerType !== "touch" && event.button !== 0)) return;
      const target = event.target instanceof Element ? event.target : null;
