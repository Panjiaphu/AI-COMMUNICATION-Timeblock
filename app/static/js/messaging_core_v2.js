(function () {
  ...
  function mountThreadToolbar() {
    ...
    const prelude = element("div", "messaging-v2-thread-prelude");
    container.before(prelude);
    mountPinnedSurface(prelude);
  ...
  function mountPinnedSurface(prelude) {
    if (ui.pinnedHeader) return;
    const header = element("section", "messaging-v2-pinned-header");
    header.hidden = true;
    header.setAttribute("aria-label", text("pinnedTitle"));
    const icon = element("span", "messaging-v2-pinned-icon", "📌");
    icon.setAttribute("aria-hidden", "true");
    const copy = element("div", "messaging-v2-pinned-copy");
    const summary = element("strong", "messaging-v2-pinned-summary");
    const preview = element("span", "messaging-v2-pinned-preview");
    copy.append(summary, preview);
    const open = actionButton(text("pinnedOpen"), "messaging-v2-button is-primary");
    open.dataset.pinnedCenterTrigger = "true";
    open.setAttribute("aria-haspopup", "dialog");
    open.addEventListener("click", () => openPinnedCenter(open));
    header.append(icon, copy, open);
    prelude.append(header);

    ui.pinnedHeader = header;
    ui.pinnedSummary = summary;
    ui.pinnedPreview = preview;
    ui.pinnedOpen = open;
    buildPinnedCenter();
  }

  function buildPinnedCenter() {
    if (ui.pinnedDialog) return;
    const dialog = element(
      "dialog",
      "messaging-v2-dialog messaging-v2-pinned-dialog",
    );
    const body = element("div", "messaging-v2-dialog-body");
    const header = dialogHeader(text("pinnedTitle"), dialog);
    const feedback = element("p", "messaging-v2-dialog-feedback");
    feedback.setAttribute("role", "status");
    const list = element("div", "messaging-v2-pinned-list");
    const empty = element("p", "messaging-v2-pinned-empty", text("pinnedEmpty"));
    empty.hidden = true;
    const loadMore = actionButton(
      text("pinnedLoadMore"),
      "messaging-v2-button messaging-v2-pinned-load-more",
    );
    loadMore.hidden = true;
    loadMore.addEventListener("click", () => refreshPinnedMessages({ append: true }));
    body.append(header, feedback, list, empty, loadMore);
    dialog.appendChild(body);
    app.appendChild(dialog);

    ui.pinnedDialog = dialog;
    ui.pinnedFeedback = feedback;
    ui.pinnedList = list;
    ui.pinnedEmpty = empty;
    ui.pinnedLoadMore = loadMore;
    dialog.addEventListener("close", () => {
      state.pinnedCenterOpen = false;
    });
  }
  ...
  function pinnedDate(value) {
    if (!value) return "";
    try {
      return new Intl.DateTimeFormat(
        app.dataset.locale || undefined,
  ...
  function pinnedTypeLabel(message) {
    const type = String(message?.content_type || "text").toLowerCase();
    const key = {
      text: "pinnedTypeText",
      image: "pinnedTypeImage",
      file: "pinnedTypeFile",
      audio: "pinnedTypeAudio",
      location: "pinnedTypeLocation",
      ptt: "pinnedTypeAudio",
      tombstone: "pinnedTypeUnavailable",
    }[type] || "pinnedTypeOther";
    return text(key) || type;
  }

  function pinnedImageCount(attachments) {
    ...
    if (Array.isArray(attachments.attachments)) return attachments.attachments.length;
    return 1;
  }

  function pinnedPreview(message) {
    if (!message || message.content_type === "tombstone" || message.deleted_for_everyone_at) {
      return text("pinnedUnavailable");
    }
    const content = String(message.content || "").trim();
    if (message.content_type === "text" && content) return content;
    if (message.content_type === "image") {
      const count = pinnedImageCount(message.attachments);
      return count > 1
        ? text("pinnedImages").replace("{count}", String(count))
        : text("pinnedTypeImage");
    }
    return content && content !== String(message.content_type || "")
      ? content
      : pinnedTypeLabel(message);
  }

  function pinnedSender(item) {
    ...
    return String(
      ...
      || text("pinnedSenderUnknown"),
    );
  }

  function renderPinnedState() {
    const count = Math.max(0, Number(state.pinnedCount) || 0);
    const latest = state.pinnedItems[0];
    ui.pinnedHeader.hidden = !state.conversation || count === 0;
    ui.pinnedSummary.textContent = text("pinnedCount").replace(
      "{count}",
      String(count),
    );
    ui.pinnedPreview.textContent = latest
      ? pinnedPreview(latest.message)
      : "";
    ui.pinnedOpen.disabled = count === 0;
    ui.pinnedOpen.setAttribute(
      "aria-label",
      `${text("pinnedOpen")} (${count})`,
    );
  }

  function renderPinnedList() {
    if (!ui.pinnedList) return;
    ui.pinnedList.replaceChildren();
    const items = Array.isArray(state.pinnedItems) ? state.pinnedItems : [];
    ui.pinnedEmpty.hidden = items.length > 0;
    items.forEach((item) => {
      const message = item.message || {};
      const row = element("article", "messaging-v2-pinned-item");
      row.dataset.pinId = String(item.pin_id || "");
      row.dataset.messageId = String(message.id || "");
      const top = element("div", "messaging-v2-pinned-item-top");
      const sender = element("strong", "messaging-v2-pinned-sender", pinnedSender(item));
      const created = element("time", "messaging-v2-pinned-time", pinnedDate(message.created_at));
      if (message.created_at) created.dateTime = String(message.created_at);
      top.append(sender, created);
      const meta = element("span", "messaging-v2-pinned-meta");
      const pinnedAt = pinnedDate(item.pinned_at);
      meta.textContent = [pinnedTypeLabel(message), pinnedAt].filter(Boolean).join(" · ");
      const preview = element("p", "messaging-v2-pinned-item-preview", pinnedPreview(message));
      const actions = element("div", "messaging-v2-pinned-item-actions");
      const jump = actionButton(text("pinnedJump"), "messaging-v2-button is-primary");
      jump.addEventListener("click", () => jumpToPinnedMessage(item));
      const unpin = actionButton(text("pinnedUnpin"), "messaging-v2-button");
      unpin.addEventListener("click", () => unpinFromPinnedCenter(item, unpin));
      actions.append(jump, unpin);
      row.append(top, meta, preview, actions);
      ui.pinnedList.appendChild(row);
    });
    ui.pinnedLoadMore.hidden = !state.pinnedPage?.has_more_before;
    renderPinnedState();
  }

  async function refreshPinnedMessages({ append = false } = {}) {
    if (!state.conversation || state.pinnedLoading) return;
    const conversationId = Number(state.conversation.id);
    const cursor = append ? state.pinnedPage?.next_before_cursor : "";
    if (append && !cursor) return;
    state.pinnedLoading = true;
    if (!append && ui.pinnedFeedback) ui.pinnedFeedback.textContent = "";
    try {
      const query = new URLSearchParams({ limit: "50" });
      if (cursor) query.set("before_pinned_cursor", cursor);
      const payload = await api(
        `/api/messaging/conversations/${encodeURIComponent(conversationId)}/pinned-messages?${query.toString()}`,
      );
      if (!state.conversation || Number(state.conversation.id) !== conversationId) return;
      const incoming = Array.isArray(payload.items) ? payload.items : [];
      const combined = append ? [...state.pinnedItems, ...incoming] : incoming;
      state.pinnedItems = Array.from(
        new Map(combined.map((item) => [String(item.pin_id), item])).values(),
      );
      state.pinnedPage = payload.page || {};
      state.pinnedCount = Number(payload.count) || 0;
      renderPinnedList();
    } catch (_error) {
      if (ui.pinnedFeedback) ui.pinnedFeedback.textContent = text("actionFailed");
      if (!append) renderPinnedState();
    } finally {
      state.pinnedLoading = false;
    }
  }

  async function openPinnedCenter(returnFocus) {
    if (!state.conversation || !ui.pinnedDialog) return;
    state.pinnedCenterOpen = true;
    showDialog(ui.pinnedDialog, returnFocus, ui.pinnedDialog.querySelector("button"));
    await refreshPinnedMessages();
  }

  async function unpinFromPinnedCenter(item, button) {
    const messageId = item?.message?.id;
    if (!messageId) return;
    button.disabled = true;
    try {
      await api(
        `/api/messaging/messages/${encodeURIComponent(messageId)}/pin`,
        { method: "DELETE" },
      );
      await refreshPinnedMessages();
      requestRefresh("thread");
  ...
  function scrollToMessage(messageId) {
    ...
    const container = messagesPanel.querySelector("[data-thread-messages]");
    const bubble = Array.from(container?.querySelectorAll("[data-message-id]") || [])
      .find((node) => String(node.dataset.messageId || "") === target);
    if (!bubble) return false;
    bubble.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
    const previousTabIndex = bubble.getAttribute("tabindex");
    bubble.setAttribute("tabindex", "-1");
    bubble.classList.remove("is-pinned-target");
    void bubble.offsetWidth;
    bubble.classList.add("is-pinned-target");
    bubble.focus({ preventScroll: true });
    window.setTimeout(() => {
      bubble.classList.remove("is-pinned-target");
      if (previousTabIndex === null) bubble.removeAttribute("tabindex");
      else bubble.setAttribute("tabindex", previousTabIndex);
    }, 1600);
    return true;
  }

  function jumpToPinnedMessage(item) {
    const messageId = item?.message?.id;
    if (!messageId) return;
    closeDialog(ui.pinnedDialog);
    if (scrollToMessage(messageId)) return;
    emit("jump-to-message", { messageId: Number(messageId) });
  }
  ...
  function messageActions(message) {
    ...
    if (isMine(message) && message.content_type === "text") {
  ...
  async function pollEvents() {
    ...
    try {
      for (let page = 0; page < 5; page += 1) {
        ...
        if (events.length) {
          events.forEach((item) => {
            if (!rememberEventId(item.event_id)) return;
            if (!state.eventBootstrapping) {
              sawNewEvent = true;
              emitCallState(item);
              emitCallTranslation(item);
  ...
  function refreshScopeForEvent(payload) {
    ...
    if (eventType.startsWith("translation.")) return "";
  ...
  function emitCallTranslation(payload) {
    const eventType = String(payload?.event_type || "");
    if (eventType.startsWith("translation.")) emit("call-translation", { event: payload, eventType });
  }
  ...
  function bindMessagingEvents() {
    ...
    app.addEventListener("timeblock:messaging:jump-result", (event) => {
      const messageId = Number(event.detail?.messageId || 0);
      if (!messageId || !event.detail?.found) {
        showNotice(text("pinnedUnavailable"), true);
        return;
      }
