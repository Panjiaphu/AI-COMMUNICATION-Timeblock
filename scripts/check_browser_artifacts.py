...
REQUIRED_VIEWPORTS = (
    'mobile-390x844', 'mobile-landscape-844x390', 'mobile-430x932', 'mobile-landscape-932x430',
    'tablet-768x1024', 'tablet-landscape-1024x768', 'desktop-1366x768', 'desktop-1440x900',
)
REQUIRED_EVIDENCE_FILES = (
    *(f'evidence/{name}.json' for name in REQUIRED_VIEWPORTS),
    'evidence/fake-media-granted.json', 'evidence/permission-denied.json',
    'evidence/webrtc-reconnect.json', 'evidence/webrtc-participant-a.json',
    'evidence/webrtc-participant-b.json', 'evidence/webrtc-participant-c.json',
    'evidence/reconnect-exhaustion.json', 'evidence/one-sided-hangup.json',
)
REQUIRED_SCREENSHOT_FILES = (
    *(f'screenshots/{name}.png' for name in REQUIRED_VIEWPORTS),
    'screenshots/webkit-mobile-390x844.png', 'screenshots/fake-media-cleanup.png',
    'screenshots/webrtc-participant-a.png', 'screenshots/webrtc-participant-b.png',
)
REQUIRED_TRACE_FILES = ('traces/mobile-390x844.zip', 'traces/desktop-1366x768.zip')
SERVER_LOG_ALLOWED_FIELDS = {
    'timestamp', 'level', 'logger', 'message', 'result', 'trace_id', 'session_id', 'room_id',
    'workspace_id', 'participant_id', 'connection_id', 'event_name', 'event_version', 'error_code',
    'deployment_version', 'duration_ms',
}
SERVER_LOG_REQUIRED_FIELDS = {'timestamp', 'level', 'logger', 'message'}
FORBIDDEN_PATTERNS = {
    'raw reconnect token JSON field': re.compile(r'"reconnect_token"\s*:\s*"(?!\[REDACTED\]"(?:\s*[,}]))[^"\r\n]*"', re.IGNORECASE),
    'raw session token JSON field': re.compile(r'"session_token"\s*:\s*"(?!\[REDACTED\]"(?:\s*[,}]))[^"\r\n]*"', re.IGNORECASE),
    ...
    'raw reconnect token assignment': re.compile(r'\breconnect[_-]?token\s*[:=]\s*(?!\[REDACTED\](?:[&\s,;\"\']|$))[^&\s,;]+', re.IGNORECASE),
    'raw session token assignment': re.compile(r'\bsession[_-]?token\s*[:=]\s*(?!development-session(?:[&\s,;\"\']|$)|\[REDACTED\](?:[&\s,;\"\']|$))[^&\s,;]+', re.IGNORECASE),
    'raw reconnect query token': re.compile(r'(?:^|[?&])reconnect_token=(?!\[REDACTED\](?:[&\s\"\']|$))[^&\s\"\']+', re.IGNORECASE),
    'raw session query token': re.compile(r'(?:^|[?&])token=(?!development-session(?:[&\s\"\']|$)|\[REDACTED\](?:[&\s\"\']|$))[^&\s\"\']+', re.IGNORECASE),
}
...
def _validate_build_identity(path: Path) -> list[str]:
    ...
    required = {'expected_pr_head', 'checked_out_sha', 'deployment_version', 'github_event_name', 'workflow_run_id'}
    errors: list[str] = []
    if required - payload.keys():
        errors.append(f'build identity is missing required fields in {path}')
        return errors
    expected_head = str(payload['expected_pr_head']); checked_out_sha = str(payload['checked_out_sha']); deployment_version = str(payload['deployment_version'])
...
def main() -> int:
    ...
    if not root.exists():
        ...
        return 1
    required_groups = {
        'build identity': list(root.glob('build-identity.json')), 'browser versions': list(root.glob('browser-versions.json')),
        'screenshots': list((root / 'screenshots').glob('*.png')), 'traces': list((root / 'traces').glob('*.zip')),
        'evidence': list((root / 'evidence').glob('*.json')), 'JUnit': list(root.glob('junit.xml')), 'server log': list(root.glob('server.log')),
    }
